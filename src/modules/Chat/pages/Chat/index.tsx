...
import "./style.css";
import { ChatHeader, ChatInfo, ClassifyChat, Welcome, JoinGroupModal } from "./components";
...
const Chat = () => {
    ...
    useEffect(() => {
        if (!isJoinChatLayout) {
            ...
            socket.on("update-avatar-conversation", (conversationId, conversationAvatar) => {
                if (refCurrentConversation.current === conversationId) {
                    dispatch(
                        updateAvavarConver({
                            conversationId,
    ...
    const emitUserOnline = useCallback(
        (currentConversation: string) => {
            if (currentConversation) {
                const conver = conversations.find((ele) => ele._id === currentConversation);
                if (!conver?.type) {
