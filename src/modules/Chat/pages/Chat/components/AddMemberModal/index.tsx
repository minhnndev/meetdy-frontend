...
import { Checkbox, Input, LocaleProvider, Modal, Toast, Transfer } from "@douyinfe/semi-ui";
...
const AddMemberModal = ({ visible, onCancel, conversationType }) => {
    ...
    const { memberInConversation, currentConversation } = useAppSelector((state) => state.chat);
    ...
    const handleOk = async () => {
        setLoading(true);
        const selectedIds = selectedFriendIds?.filter((id) => !checkInConversation(id));
        if (conversationType) {
            await ServiceConversation.addMembersToConver(selectedIds, currentConversation);
