...
import { Button, Dropdown, Modal, Nav, Toast, Typography } from "@douyinfe/semi-ui";
...
const FriendItem = ({ friend }: { friend: TFriend }) => {
    ...
    const handleClickItem = async () => {
        const response = await ServiceConversation.createConversationIndividual(friend._id);
        const { _id, isExists } = response;

        if (!isExists) {
            const conversation = await ServiceConversation.getConversationById(friend._id);
