...
const SuggestCard = ({ suggest }: { suggest: TSuggestFriend }) => {
    ...
    const { avatar, avatarColor, name, numberCommonFriend, numberCommonGroup } = suggest;
    return (
        <>
            <Card shadows="hover" style={{ width: 248 }}>
                ...
                >
                    <UserAvatar avatar={avatar} color={avatarColor} name={name} size="large" />
                    <Typography.Title
                        heading={6}
            ...
            <UserCard visible={showUser} onCancel={() => setShowUser(false)} user={suggest} />
        </>
    );
};
