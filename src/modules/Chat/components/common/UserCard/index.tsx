...
const UserCard = ({
    ...
    const handleClickMessage = async () => {
        const response = await ServiceConversation.createConversationIndividual(user._id);
    ...
    return (
        <Modal visible={visible} title="Thông tin" onCancel={onCancel} footer={null}>
            <Image src={coverImage} width={400} height={180} style={{ objectFit: "cover" }} />
            <div className="flex-center" style={{ marginTop: -40 }}>
                {avatar ? (
                    ...
                    <UserAvatar avatar="" name={user.name} size="large" color={avatarColor} />
                )}
            </div>
            <Paragraph className="flex-center" style={{ fontSize: 20, margin: "1rem 0" }}>
                {name}
            </Paragraph>
            <div className="flex-center">
                ...
                <Button theme="outline" type="tertiary" size="large" onClick={handleClickMessage}>
                    Nhắn tin
                </Button>
            </div>
