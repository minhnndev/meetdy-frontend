...
const Chat = (props: ChatProps) => {
    ...
    const { isJoinChatLayout, isJoinFriendLayout, user } = useSelector((state: any) => state.global);
    ...
    useEffect(() => {
        if (currentConversation) {
            dispatch(setTotalChannelNotify(currentConversation));
        }
    ...
    useEffect(() => {
        const openModalJoinFromLink = async () => {
            if (location.state && location.state.conversationId) {
                ...
                //             content: 'Trưởng nhóm đã tắt tính năng tham gia nhóm bằng liên kết',
                //         });
                //     }
                // } else {
    ...
    useEffect(() => {
        if (singleConversations) {
            setSingleConverFilter(singleConversations)
        }
    }, [singleConversations])

    useEffect(() => {
        if (groupConversations) {
            setMutipleConverFilter(groupConversations)
        }
    }, [groupConversations])

    const handleOnSubmitSearch = async (value: string) => {
        try {
            await dispatch(fetchListConversations({name: value, type: 1}) as any);
            await dispatch(fetchListConversations({name: value, type: 2}) as any);
        } catch (error) {}
    };
                            ...
                            {visibleFilter ? (
                                ...
                                <>
                                    ...
                                    <div className="main-conversation_list-conversation">
                                        <ConversationContainer conversations={conversations} valueClassify={valueClassify} />
                                    </div>
                                </>
                            )}
                        </div>
