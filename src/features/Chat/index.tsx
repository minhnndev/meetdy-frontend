...
import { setJoinChatLayout } from "@/store/globalSlice";
import FilterContainer from "@/components/legacy/FilterContainer";
import ModalJoinGroupFromLink from "@/components/legacy/ModalJoinGroupFromLink";
...
import renderWidthDrawer from "@/utils/utilsLegacy/DrawerResponsive";
...
function Chat({ socket, idNewMessage }) {
    ...
    const { conversations, currentConversation, pinMessages, isLoading, currentChannel, channels } =
    ...
    const { isJoinChatLayout, isJoinFriendLayout, user } = useSelector((state) => state.global);
        ...
        const openModalJoinFromLink = async () => {
            if (location.state && location.state.conversationId) {
                ...
                if (data.findIndex((ele) => ele._id === tempId) < 0) {
                    try {
                        const data = await conversationApi.fetchSummaryInfoGroup(tempId);
                        setSummary(data);
    ...
    useEffect(() => {
        if (!isJoinChatLayout) {
            socket.on("delete-conversation", (conversationId) => {
                const conver = refConversations.current.find((ele) => ele._id === conversationId);
            ...
            socket.on("add-reaction", ({ conversationId, channelId, messageId, user, type }) => {
                if (
            ...
            socket.on("rename-conversation", (conversationId, conversationName, message) => {
                dispatch(updateNameOfConver({ conversationId, conversationName }));
                dispatch(addMessage(message));
            });

            socket.on("user-last-view", ({ conversationId, userId, lastView, channelId }) => {
                if (userId !== user._id) {
                    dispatch(
                        updateLastViewOfMembers({
                            conversationId,
    ...
    const handleOnSubmitSearch = async () => {
        try {
            const single = await conversationApi.fetchListConversations(valueInput, 1);
            setSingleConverFilter(single);
            const mutiple = await conversationApi.fetchListConversations(valueInput, 2);
