import { DeleteOutlined, PushpinOutlined, UndoOutlined } from "@ant-design/icons";
import { Button, Dropdown, Menu, message as mesageNotify } from "antd";
import messageApi from "@/api/messageApi";
import pinMessageApi from "@/api/pinMessageApi";
import ModalChangePinMessage from "@/components/legacy/ModalChangePinMessage";
import MESSAGE_STYLE from "@/utils/constants/MessageStyle/messageStyle";
...
import { checkLeader } from "@/utils/utilsLegacy/groupUtils";
...
function UserMessage({
    ...
    const { _id, content, user, createdAt, type, isDeleted, reacts, tagUsers, replyMessage } =
        message;
    ...
    const myReact =
        reacts && reacts.length > 0 && reacts.find((ele) => ele.user._id === global.user._id);
            ...
            {!isDeleted && type === "NOTIFY" ? (
                <>
                    ...
                    <div className="last-view-avatar center">
                        {viewUsers && viewUsers.length > 0 && <LastView lastView={viewUsers} />}
                    </div>
                </>
            ) : (
                <>
                    ...
                    >
                        <div className={`interact-conversation ${isMyMessage ? "reverse" : ""}  `}>
