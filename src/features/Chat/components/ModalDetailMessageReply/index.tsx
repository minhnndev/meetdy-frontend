...
import ModalVideoCustom from "@/components/legacy/ModalVideoCustom";
import OverlayImage from "@/components/legacy/OverlayImage";
import parse from "html-react-parser";
import PropTypes from "prop-types";
import React, { useState } from "react";
import { defaultStyles, FileIcon } from "react-file-icon";
import fileHelpers from "@/utils/utilsLegacy/fileHelpers";
...
function ModalDetailMessageReply({ visible, onCancel, data }) {
    ...
    return (
        <Modal visible={visible} footer={null} onCancel={handleCancel} closable={false}>
                ...
                <div className="reply-info">
                    ...
                    <div className="time-user">
                        ...
                        <div className="reply-time">
                            ...
                            &nbsp;
                            <span>
                                {`0${time.getDate()}`.slice(-2)}/{`0${time.getMonth()}`.slice(-2)}/
                                {`${time.getFullYear()}`}
                            </span>
                        </div>
                    </div>
