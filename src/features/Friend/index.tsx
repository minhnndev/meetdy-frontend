...
import { getValueFromKey } from "@/utils/constants/filterFriend";
import SearchContainer from "@/features/Chat/containers/SearchContainer";
import PropTypes from "prop-types";
import React, { useEffect, useRef, useState } from "react";
import Scrollbars from "react-custom-scrollbars";
import { useDispatch, useSelector } from "react-redux";
import { sortGroup } from "@/utils/utilsLegacy/groupUtils";
...
function Friend({ socket }) {
    ...
    const handleMenuLeftSelect = ({ _, key }) => {
        if (groups.length > 0) {
            ...
            if (key === "2") {
                const newGroup = groupCurrent.filter((ele) => ele.leaderId === user._id);
                    ...
                    >
                        <div className="main-friend_body">
                            <div className="main-friend_body__header">
                                <HeaderFriend onBack={() => setActiveTab(false)} subtab={subTab} />
                            </div>
