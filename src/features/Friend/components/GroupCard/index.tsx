...
import SubMenuClassify from "@/components/legacy/SubMenuClassify";
...
import classifyUtils from "@/utils/utilsLegacy/classifyUtils";
import "./style.css";
...
function GroupCard({ data, onRemove }) {
    ...
    const mainCard = (
        <Dropdown overlay={menu} trigger={["contextMenu"]}>
            <div className="group-card">
                ...
                <div className="group-card__total-member">{`${data.totalMembers} thành viên`}</div>
    ...
    return (
        <div onClick={handleOnClick}>
            {classify ? (
                <Badge.Ribbon text={classify.name} color={classify.color.code} placement="start">
                    {mainCard}
                </Badge.Ribbon>
            ) : (
                mainCard
